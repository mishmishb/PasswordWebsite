// Transcrypt'ed from Python, 2019-06-09 21:45:05
var itertools = {};
...
export var generate_l33t_list = function (password, l33t_list) {
	...
	for (var n_a of nonalpha) {
		var swapped_letters = (function () {
			var __accu0__ = [];
			for (var [lettrs, l33ts] of l33t_list.py_items ()) {
				if (__in__ (n_a, l33ts)) {
...
export var l33t_finder = function (password, ranked_wordlists) {
	var found_words = [];
	var l33t_list = generate_l33t_list (password, L33T_CHARACTERS);
	var password_options = [];
	for (var [position, value] of enumerate (password)) {
		var is_l33t = false;
		for (var [l33t, letter] of l33t_list.py_items ()) {
			if (l33t == value) {
	...
	for (var permutation of password_permutations) {
		for (var word of dictionary_finder (permutation, ranked_wordlists)) {
			if (word ['length'] > 1) {
				...
				for (var [l33t, letter] of l33t_list.py_items ()) {
					var swap_list = [];
					if (__in__ (l33t, sequence)) {
						for (var l of letter) {
							if (!__in__ (l, sequence)) {
								swap_list.extend (l);
							}
						}
						if (swap_list) {
